%% CREATE_MISSING_REPORT_FIGURES_V3
...
SETTINGS.RUN_IMPLICIT = false;
SETTINGS.SAVE_FIGURES = true;
SETTINGS.SAVE_MAT = true;

SETTINGS.RUNTIME_REPEATS = 5; % repetir tiempos para reducir ruido
...
SETTINGS.MESH_LIST = [66, 130, 258, 514, 1026, 2050, 4098];
...
SETTINGS.VERIFICATION_MESH = 4098;
SETTINGS.COMPARISON_MESH = 514;
SETTINGS.REFINEMENT_PLOT_SKIP = [1026, 2050];
...
%% =========================
%  FIGURA 4: COSTE COMPUTACIONAL EXPLÍCITO
% ==========================
fig = figure('Name', 'cost_explicit', 'Color', 'w');
plot(err_rus.ncells, err_rus.runtime, 'o-', 'LineWidth', 1.8, 'MarkerSize', 7); hold on;
plot(err_hllc.ncells, err_hllc.runtime, 's-', 'LineWidth', 1.8, 'MarkerSize', 7);

% Referencias cuadráticas escaladas en una malla media para que se vea la tendencia
...
plot(nref, ref_rus, '--', 'LineWidth', 1.2);
plot(nref, ref_hllc, '--', 'LineWidth', 1.2);

grid on;
xlabel('Número de celdas [-]');
ylabel('Tiempo de cálculo [s]');
title('Coste computacional explícito');
legend('Rusanov', 'HLLC', 'Referencia cuadrática (Rusanov)', 'Referencia cuadrática (HLLC)', 'Location', 'northwest');
save_figure(fig, SETTINGS, 'fig04_coste_computacional_explicito');

%% =========================
%  FIGURA 5: ORDEN DEL COSTE COMPUTACIONAL
% ==========================
...
semilogx(cost_ord_rus.h_plot, cost_ord_rus.p_plot, 'o-', 'LineWidth', 1.8, 'MarkerSize', 7); hold on;
semilogx(cost_ord_hllc.h_plot, cost_ord_hllc.p_plot, 's-', 'LineWidth', 1.8, 'MarkerSize', 7);
yline(2, '--', 'LineWidth', 1.1);
grid on;
xlabel('h [-]');
ylabel('Orden del coste [-]');
title(sprintf('Evolución del orden del coste computacional (ajuste global: Rusanov %.2f, HLLC %.2f)', fit_rus.p, fit_hllc.p));
...
%% =========================
%  FIGURA 11: COSTE EXPLÍCITO vs IMPLÍCITO (OPCIONAL)
% ==========================
if SETTINGS.RUN_IMPLICIT
    imp_ncells = zeros(numel(implicit_cases), 1);
    imp_runtime = zeros(numel(implicit_cases), 1);

    for k = 1:numel(implicit_cases)
        imp_ncells(k) = implicit_cases{k}.ncells;
        imp_runtime(k) = implicit_cases{k}.runtime;
    end

    fig = figure('Name', 'explicit_vs_implicit_cost', 'Color', 'w');
    loglog(err_rus.ncells, err_rus.runtime, 'o-', 'LineWidth', 1.8, 'MarkerSize', 7); hold on;
    loglog(imp_ncells, imp_runtime, 'd-', 'LineWidth', 1.8, 'MarkerSize', 7);
    grid on;
    xlabel('Número de celdas [-]');
    ylabel('Tiempo de cálculo [s]');
    title('Rusanov explícito vs implícito FD');
    legend('Explícito', 'Implícito FD', 'Location', 'best');
    save_figure(fig, SETTINGS, 'fig11_coste_explicito_vs_implicito');
end
